...
import { fetchTeacherCourses } from '../redux/courseActions.js';

export const TeacherProfile = () => {
    const dispatch = useDispatch();
    const { id } = useParams();
    const user = useSelector(state => state.course.user.user);
    const courses = useSelector(state => state.course.course.teacherCourses);
    console.log(courses)
    useEffect(() => {
        dispatch(fetchTeacherProfile(id));
        dispatch(fetchTeacherCourses(id));
    }, [dispatch, id]);
            ...
            <div className="mx-auto container">
                <div className="grid grid-cols-10 gap-4">
                    <div className="col-span-7 p-4">
                        ...
                        <TeacherCourseSection courses={courses} />
                    </div>
                    <div className="col-span-3 p-4">
                        <div className="bg-gray-50 border shadow-sm p-8 relative font-sans justify-center flex flex-col gap-2">
                            <button className="bg-blue-indigo text-white p-2 text-sm justify-center rounded-lg">Message</button>
                                ...
                                <div className="w-1/2">
                                    <div className="w-full bg-gray-200 rounded-full h-1.5 mt-4 relative">
                                        <div className="flex gap-2 mb-4">
                                            ...
                                            <span className="text-[11px] mb-4 text-neutral-600 absolute right-[-22px] top-[-5px]">5</span>
                                        </div>
                                    </div>
                                    <div className="w-full bg-gray-200 rounded-full h-1.5 mt-4 relative">
                                        <div className="flex gap-2 mb-4">
                                            <div className="bg-yellow-400 h-1.5 rounded-full w-2/3"></div>
                                            <span className="text-[11px] mb-4 text-neutral-600 absolute right-[-22px] top-[-5px]">4</span>
                                        </div>
                                    </div>
                                    <div className="w-full bg-gray-200 rounded-full h-1.5 mt-4 relative">
                                        <div className="flex gap-2 mb-4">
                                            <div className="bg-yellow-400 h-1.5 rounded-full w-1/5"></div>
                                            <span className="text-[11px] mb-4 text-neutral-600 absolute right-[-22px] top-[-5px]">3</span>
                                        </div>
                                    </div>
                                    <div className="w-full bg-gray-200 rounded-full h-1.5 mt-4 relative">
                                        <div className="flex gap-2 mb-4">
                                            <div className="bg-yellow-400 h-1.5 rounded-full w-3/4"></div>
                                            <span className="text-[11px] mb-4 text-neutral-600 absolute right-[-22px] top-[-5px]">2</span>
                                        </div>
                                    </div>
                                    <div className="w-full bg-gray-200 rounded-full h-1.5 mt-4 relative">
                                        <div className="flex gap-2 mb-4">
                                            <div className="bg-yellow-400 h-1.5 rounded-full w-1/3"></div>
                                            <span className="text-[11px] mb-4 text-neutral-600 absolute right-[-22px] top-[-5px]">1</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
