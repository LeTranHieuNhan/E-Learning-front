...
export const CourseDetailComponent = ({course}) => {
    ...
    const handleEnroll = async () => {
        ...
        if (!course?.id || !user?.id) {
            setToastMessage("Course or user information is missing.");
            setShowToast(true);
            return;
        }

        try {
            const response = await axiosInstance.post(`http://localhost:8080/api/v1/course_enrollments/courses/users/enroll?courseId=${course.id}&userId=${user.id}`, null, {
            });

            if (response.status === 204) { // No Content status
                navigate(`/course/${course.id}/1`);
            } else {
                setToastMessage(`Failed to enroll in the course: ${response.statusText}`);
                setShowToast(true);
            }
        } catch (error) {
            setToastMessage(`An error occurred while enrolling in the course: ${error.message}`);
            setShowToast(true);
        }
    };
        ...
        <div>
            ...
            <div className="flex justify-between mb-6">
                <div className="flex gap-1 mb-8">
                    <StarIcon/>
                    <span
                        className="font-sans text-sm font-bold">{course?.averageRating == null ? 4.5 : course.averageRating.toFixed(2)}</span>
                    <span
                        className="font-sans text-sm text-neutral-500">({course?.totalReviews == null ? 0 : course.totalReviews} reviews)</span>
            ...
            <div className="flex justify-between">
                ...
                <div className="mt-4 absolute left-2/3 z-0">
                    ...
                    <div className="bg-neutral-50 w-[380px] p-6 mt-5 z-0">
                        ...
                        <button
                            className="w-full mt-5 border border-blue-indigo h-10 font-sans font-bold bg-white text-blue-indigo">
                            Add to Waitlist
                        </button>
                    </div>
                </div>
