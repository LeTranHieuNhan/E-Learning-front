...
const CourseVideoComponent = ({ courseSessions }) => {
    const [currentVideoUrl, setCurrentVideoUrl] = useState(courseSessions[0]?.videoUrl);
    ...
    return (
        <div>
            <div className='flex justify-between'>
                <div>
                    <h2 className="font-sora text-[32px] font-bold text-neutral-900">{courseSessions[0]?.course?.title}</h2>
                    <div className="text-sm font-sans flex gap-2">
                        <span className="text-blue-indigo cursor-pointer">{courseSessions[0]?.course?.user?.name}</span>
            ...
            <div className="grid grid-cols-10 gap-4 mt-4">
                ...
                <div className="col-span-3 p-4">
                    ...
                    {courseSessions.map((session, index) => (
                        <div
                            key={session.id}
                            className={`flex justify-between mt-3 font-sora cursor-pointer py-4 px-2 ${session.isWatching ? 'bg-[#FFF4F0FF] text-[#F84B01FF]' : 'text-[#9095A0FF]'}`}>
                            <h2 className='text-sm'>{`${session.sessionOrder}. ${session.title}`}</h2>
                            {session.isWatching ? (
